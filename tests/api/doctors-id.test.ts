...
describe('API: /api/doctors/[id]', () => {
  test('returns 400 when id is missing', async () => {
    const req = {} as any
    // @ts-expect-error params type in route is Promise
    const { GET } = require('@/app/api/doctors/[id]/route')
    const res = await GET(req, { params: Promise.resolve({ id: '' }) })
    expect(res.status).toBe(400)
    const body = await res.json()
    expect(body).toEqual({ error: 'Doctor ID is required' })
  })
  ...
  test('returns 200 with entry when found', async () => {
    ...
    getClient.mockReturnValue({
      getEntry: jest.fn().mockResolvedValue({ id: 'abc', fields: { doctorName: 'John' } }),
    })
    ...
    const body = await res.json()
    expect(body).toEqual({ id: 'abc', fields: { doctorName: 'John' } })
